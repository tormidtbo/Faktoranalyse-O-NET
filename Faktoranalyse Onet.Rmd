...
data <- as_tibble(read.delim("C:\\Users\\torm\\OneDrive - Kompetanse Norge\\Dokumenter\\GitHub\\Onet\\Faktoranalyse-O-net\\onet_rdatasett.tsv", encoding = "UTF-8"))
...
Splitte datasettet per område. Evt prøve modell som utelater fysiske egenskapene ettersom de driver mye av variasjonen i modellene? Fjerne 
...
```
Forsøk med å utelate kunnskaper
```{r}
all.uk <- da[,-grep("kunnskaper", names(da))]
S.2 <- cor(all.uk, method = "pearson")
fa.parallel(x= S.2, n.obs = 301)


(efamodel4.uk <- fa(r=S.2, nfactors = 4, fm='minres', n.obs = 301)) #foretrukket modell
(efa4.uk <- Ind_per_factor(efamodel4.uk, 10, 5))
